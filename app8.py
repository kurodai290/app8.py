...
def start_new_game(player_names):
    # 生きているプレイヤーのリストを作成
    st.session_state.players = [{"name": n, "alive": True} for n in player_names]
    ...
    st.session_state.logs = [f"【{len(player_names)}人】での攻略開始。最後まで生き残れ。"]
...
def next_turn():
    ...
    while True:
        ...
        if st.session_state.players[st.session_state.turn_idx]["alive"]:
            break
        if st.session_state.turn_idx == original_idx: # 全員死んでいる場合（念のため）
            break
    st.session_state.start_time = time.time()
...
def move_car(use_mask, time_out=False):
    ...
    if car in st.session_state.gas_cars:
        if use_mask and random.random() > 0.05:
            st.session_state.logs.insert(0, f"【{car}号車】{current_p['name']}: {action_msg}。毒ガスを耐えた。")
        else:
            st.session_state.players[p_idx]["alive"] = False
            st.session_state.logs.insert(0, f"【{car}号車】💀 {current_p['name']} が死亡。")
    else:
        st.session_state.logs.insert(0, f"【{car}号車】{current_p['name']}: {action_msg}。空気は正常。")
...
if st.session_state.game_status == "START_SCREEN":
    ...
    # 生存者リスト表示
    alives = ", ".join([p["name"] for p in st.session_state.players if p["alive"]])
    st.caption(f"生存者: {alives}")
    ...
    st.success(f"🎉 げぇむくりあ！ 生還者: {winners}")
